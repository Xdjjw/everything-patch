...
function testEnv(dataset) {
  ...
  if (dataset) kv.values.set("repository:xdjjw/everything-patch", { value: JSON.stringify(dataset), metadata: null });
  return {
    STAR_HISTORY: kv,
    ALLOWED_REPOSITORIES: "Xdjjw/everything-patch",
    REPOSITORY_ALIASES: "everything-patch=Xdjjw/everything-patch",
...
function baseline(checkedAt = new Date().toISOString()) {
  return buildBaseline({
    repository: "Xdjjw/everything-patch",
...
test("chart responses support ETag revalidation with multiple validators", async () => {
  ...
  const first = await worker.fetch(new Request("https://example.test/v1/charts/everything-patch.svg"), env);
  const etag = first.headers.get("ETag");
  assert.equal(first.status, 200);
  assert.ok(etag?.startsWith('W/"v2-'));
  assert.match(await first.text(), /^<svg/);

  const second = await worker.fetch(new Request("https://example.test/v1/charts/everything-patch.svg", {
...
test("refresh ingests an Actions snapshot and stores aggregated history", async () => {
  ...
  const stored = await env.STAR_HISTORY.get("repository:xdjjw/everything-patch", "json");
...
test("webhook events use unique keys and duplicate deliveries stay idempotent", async () => {
  ...
  const payload = JSON.stringify({
    ...
    repository: { full_name: "Xdjjw/everything-patch", stargazers_count: 11 },
  });
  ...
  const data = await worker.fetch(new Request("https://example.test/v1/data/everything-patch"), env);
