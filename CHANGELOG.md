...
- 升级后会自动暂停 Everything Patch 此前启用的皮肤并恢复官方显示；若自动停用失败，应用会显示明确错误，不会让用户在入口隐藏后失去恢复提示。
...
- 修复 macOS 皮肤已经注入后 Everything Patch 仍长期显示“应用中”、重开后无法识别当前主题的问题；单次注入现在会明确退出，外部命令带有硬超时，并可从经过验证的 Codex 主进程恢复遗失的本机调试端口。
...
- 修复从 cc-switch 重复导入时可能覆盖 Everything Patch 中已有供应商名称、模型或自定义配置的问题，并兼容旧版 cc-switch 数据。
...
- 会话管理新增指定会话永久删除：支持单选、多选，并在二次确认后从当前 Codex 存储中硬删除；操作不进入回收站、不创建新的删除备份，Everything Patch 内无法撤销。
...
- 修复追加提示词后禁用会误影响用户原有规则的风险；禁用只删除 Everything Patch 管理的 AGENTS 区块或指令文件，用户其他内容保持不变。
...
- 将 `AGENTS.md` 纳入 Everything Patch 备份与恢复流程。
...
- 修复启用第三方供应商后，关闭并重新打开 Everything Patch 不再显示“当前启用”的问题。
...
- 会话管理新增“启动自动修复”开关：开启后，Everything Patch 启动时会在后台检查本地会话完整性，发现未同步会自动修复，不阻塞界面。
- 优化按钮点击体验：移除容易造成 WebView 闪烁/掉帧的水波纹、位移和 filter 效果，点击反馈更轻、更稳。

### 修复 Bug

- 修复同名供应商可能同时显示为“当前启用”的问题；Everything Patch 现在会记录最后启用的供应商 ID，避免第三方 live Provider 都是 `custom` 时出现双亮。
...
- MCP 管理会读写 `~/.codex/config.toml` 的 `[mcp_servers]`，禁用后保留到 Everything Patch SQLite，后续可一键重新启用。
- Skills 管理会扫描 `~/.codex/skills`，并可从 `~/.agents/skills`、`~/.cc-switch/skills` 导入到 Codex；禁用后移动到 Everything Patch 禁用目录，避免直接删除。
...
- Windows Release 新增 portable ZIP，包含可直接运行的 `Everything Patch.exe`，无需 MSI 安装。
