  ...
  return (
    ...
    >
      ...
      <div className="cx-app-toast-copy">
        <strong>{firstLine || (isError ? (lang === "zh" ? "操作失败" : "Action failed") : "DevConduit")}</strong>
...
export function UpdateDialog({
  ...
  const copy = isChinese
    ? {
        ...
        downloadingDescription: "请保持 DevConduit 打开，下载完成后会自动安装。",
        installingTitle: "正在安装更新",
        installingDescription: "即将完成，请暂时不要关闭软件。",
        readyTitle: "更新已准备好",
        readyDescription: "重新启动 DevConduit 即可使用新版本。",
        errorTitle: "更新没有完成",
    ...
    : {
        ...
        downloadingDescription: "Keep DevConduit open. Installation starts automatically after download.",
        installingTitle: "Installing update",
        installingDescription: "Almost done. Please keep the app open.",
        readyTitle: "Update is ready",
        readyDescription: "Restart DevConduit to use the new version.",
        errorTitle: "Update did not finish",
        errorDescription: updaterState.failure === "restart"
          ? "DevConduit could not restart. Please try again."
          : "Try again, or use the download page if the problem continues.",
        idleTitle: "DevConduit is up to date",
    ...
    <ModalShell
      ...
      footer={(
        <>
          ...
          <Button icon={<CheckCircle2 size={16} />} onClick={onEnter}>{isChinese ? "进入 DevConduit" : "Enter DevConduit"}</Button>
        </>
      )}
    >
