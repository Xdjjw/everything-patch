...
function getCopy(lang: Lang, toolName: string) {
  return lang === "zh"
    ? {
        ...
        noDownload: "Everything Patch 只校验路径并修改配置，不会下载或运行文件。目标工具加载配置后会启动该 MCP；第三方代码、依赖和应用插件均由你手动安装。",
    ...
    : {
        ...
        noDownload: "Everything Patch only validates paths and updates configuration; it does not download or run files. The target tool starts the MCP when it loads the configuration. You install all third-party code, dependencies, and application plugins manually.",
